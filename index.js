...
console.log(chalk.bold.cyan('║                                                            ║'))
console.log(chalk.bold.magentaBright('║                    🔊 𝐔𝐋𝐓𝐑𝐀 𝐁𝐎𝐓 🔊                         ║'))
console.log(chalk.bold.cyan('║                                                            ║'))
console.log(chalk.bold.cyan('╚════════════════════════════════════════════════════════════╝'))
console.log(chalk.bold.yellowBright('\n🚀 Iniciando el mejor bot del momento...\n'))
say('ULTRA BOT', {
font: 'chrome',
align: 'center',
...
do {
phoneNumber = await question(chalk.bgBlack(chalk.bold.greenBright(`☪︎ Ingrese el número de WhatsApp.\n${chalk.bold.magentaBright('---> ')}`)))
...
console.log(chalk.bold.white(chalk.bgMagenta(`☪︎ Código de Vinculacion:`)), chalk.bold.white(chalk.white(codeBot)))
}, 3000)
}}}}
conn.isInit = false
conn.well = false
...
async function connectionUpdate(update) {
  ...
  if (update.qr != 0 && update.qr != undefined || methodCodeQR) {
    if (opcion == '1' || methodCodeQR) console.log(chalk.green.bold(`[ ☪︎ ]  Escanea este código QR`))
  }

  if (connection === "open") {
    const userJid = jidNormalizedUser(conn.user.id)
    const userName = conn.user.name || conn.user.verifiedName || "Desconocido"
    console.log(chalk.green.bold(`[ ☪︎ ]  Conectado a: ${userName}`))
    ...
    if (fs.existsSync(restarterFile)) {
      try {
        ...
        if (data.chatId) {
          await conn.sendMessage(data.chatId, { text: "✅ *𝐔𝐋𝐓𝐑𝐀 𝐁𝐎𝐓 está en línea nuevamente* 🚀" })
